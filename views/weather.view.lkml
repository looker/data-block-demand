view: weather {
  derived_table: {
    ...
    sql: WITH weather_raw AS (SELECT id,date,element,value,mflag,qflag,sflag,time FROM `bigquery-public-data.ghcn_d.ghcnd_2020`
              ...
              SELECT date,w.id,stations.longitude as longitude, stations.latitude as latitude
          ...
          ,AVG(SX35) AS SX35
          ,AVG(WT18) AS WT18
          from weather_pivoted w
           join `bigquery-public-data.geo_us_boundaries.counties` c
          on ST_CONTAINS(c.county_geom,ST_GEOGPOINT(w.longitude,w.latitude)) group by 1,2
 ;;
  }
...
dimension: tmax {
  ...
  type: number
  sql: ((${TABLE}.TMAX/10.0)*(9.0/5.0)) + 32.0 ;;
}

dimension: tmin {
  label: "Min Temperature (°F)"
  type: number
  sql: ((${TABLE}.TMIN/10.0)*(9.0/5.0)) + 32.0 ;;
}
...
measure: average_max_temparature {
  ...
  sql: ${tmax} ;;
  value_format: "#,##0.0 \" °F\""
  drill_fields: [location, average_max_temparature]
}
